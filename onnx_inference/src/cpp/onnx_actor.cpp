...
, action_(action)
{
  ...
  input_shape_ = session_.GetInputTypeInfo(0).GetTensorTypeAndShapeInfo().GetShape(); // Likely {1, 45}

  output_shape_ = session_.GetOutputTypeInfo(0).GetTensorTypeAndShapeInfo().GetShape(); // Likely {1, 12}
  ...
  input_tensor_ = std::make_unique<Ort::Value>(Ort::Value::CreateTensor<float>(
    memory_info_, observation_.data(), input_shape_.at(1), input_shape_.data(), input_shape_.size()));

  output_tensor_ = std::make_unique<Ort::Value>(Ort::Value::CreateTensor<float>(
    memory_info_, action_.data(), output_shape_.at(1), output_shape_.data(), output_shape_.size()));
};
...
void ONNXActor::act()
{
  ...
  session_.Run(run_options_, input_names, input_tensor_.get(), 1, output_names, output_tensor_.get(), 1);
}
